...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'workflowInvokeFile', help='WorkflowInvokeFile generated by the yepQCVIZ pipeline for this specific processing')
    parser.add_argument(
        'srcPath', help='Path to the Results directory created by downloadResults script')
    ...
    destinationList = [nfrPath, tssPath, tesPath, cutPath, sutPath, xutPath, trnaPath, xelementPath, centromerePath, arsPath, memePath, fimoPath,
                       allfeaturesPath, boundfeaturesPath, enrichedfeaturesPath, bamPath, peaksPath, filteredPeaksPath, subsectorPath, bigWigPath, bigBedPath, allEventsPath]

    # checking and creating destinations
    for path in destinationList:
        if os.path.isdir(path):
            print("\033[94m {} Folder exists \033[0m".format(
                path.split('/')[-1:][0]))
        else:
            print("\033[95m Creating folder : {} \033[0m".format(
                path.split('/')[-1:][0]))
            os.mkdir(path)
