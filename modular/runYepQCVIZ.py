...
to start the yepQC & Viz modular pipeline.
...
import pprint,configparser,datetime
...
def createDataLibrariesAndHistories(gi,libList,sampleList,libPrefix):
    ...
    libs = {} # to store {run:[ library_id, {sample:[folder_id,history_id,dataset_id]} ]}
    ...
    for run,samples in libList.items():
        ...
        mylib = gi.libraries.create_library(libname, description=None, synopsis=None)
        ...
        for i in range(0,len(samples)):
            ...
            folder=gi.libraries.create_folder(mylib['id'],folder_name=folderName, description=None, base_folder_id=None)
            print("\033[94m  \033[94m  Created Folder : {} \033[0m \033[0m ".format(folderName))
            ...
            print("\033[94m  \033[94m  Created History : {} with id {} \033[0m \033[0m ".format(historyName,history['id']))
...
def getLibraryDatasetsToUpload(gi,libraries):
    ...
    uploadLibDatasets = [] # to store the dataset ids after uploading into each history.
    ...
    for i in range(0,len(glibs)):
        if glibs[i]['deleted'] == False:
            if glibs[i]['name'] in libsToUpload:
                print(" \033[94m  \033[94m Found library : {},  id: {} \033[0m \033[0m ".format(glibs[i]['name'],glibs[i]['id']))
    ...
    print(" \033[94m  \033[94m Datasets Found within library : {} \033[0m \033[0m ".format(len(uploadLibDatasets)))

    return uploadLibDatasets

def uploadDataToHistories(gi,libs,sampleList,chexmixData):
    """
    ...
    for run, data in libs.items():
        ...
        for sample,idList in data[1].items():
            if sample not in uploadInfo[run].keys():
                ...
                upload_data = gi.histories.upload_dataset_from_library(history_id=idList[1],lib_dataset_id=idList[2])

                # pprint.pprint(upload_data)
                # print upload_data['name']

                # rename the de-dup bam file based on the run and sample ID
                updateDataset = gi.histories.update_dataset(history_id=idList[1], dataset_id=upload_data['id'],name=datasetName)
                ...
                for i in range(0,len(uploadLibDatasets)):
                    upload_data = gi.histories.upload_dataset_from_library(history_id=idList[1],lib_dataset_id=uploadLibDatasets[i])
                    uploadInfo[run][sample].append(upload_data)
                ...
                print("\033[94m  \033[94m  Uploaded generic datasets for sample : {} \033[0m \033[0m".format(sample))

                chexmixDatasets = chexmixData[sample]
                # Uploading the chexmix related datasets from chexmix datalibrary.
                for i in range(0,len(chexmixDatasets)):
                    upload_data = gi.histories.upload_dataset_from_library(history_id=idList[1],lib_dataset_id=chexmixDatasets[i])
                    uploadInfo[run][sample].append(upload_data)
                    # print("run : {} sample :{} uploaddatasetid : {} upload dataset source: {} upload dataset name: {} history id: {}".format(run,sample,upload_data['id'],upload_data['hda_ldda'],upload_data['name'],idList[1]))
                print("\033[94m  \033[94m  Uploaded chexmix datasets for sample : {} \033[0m \033[0m".format(sample))
    ...
    for libDeleteId in libIdToDelete:
        ...
        print("\033[94m  \033[94m  Deleted the Library with id : {} \033[0m \033[0m".format(libDeleteId))
...
def getInputsAndParams(gi,uploadInfo,libs,workflowID,confInputs,confParams,sampleList):
    ...
    print("\033[94m  \033[94m  Extracted the RAW INPUT DICT and PARAMS DICT for : {} \033[0m \033[0m".format(workflowID['name']))
    ...
    for run,data in uploadInfo.items():
        ...
        for sample,datasets in data.items():
            print("\033[94m  \033[94m  sample : {} \033[0m \033[0m".format(sample))
            ...
            inputDict = {} # temp
            for i in range(0,len(datasets)):
                inputDict[datasets[i]['name']] = {'id':datasets[i]['id'],'src':datasets[i]['hda_ldda']}
            ...
            for k,v in raw_inputs.items():
                ...
                if raw_inputs[k]['label'] in confInputs.keys():
                    ...
                    # Creating the correct input for chexmix_peaks for each protein
                    elif label == 'chexmix_peaks':
                        datasetName = sample+"_chexmix_peaks.bed"
                        inputs[k] = inputDict[datasetName]
                    # Creating the correct input for chexmix_allevents for each protein
                    elif label == 'chexmix_allevents':
                        datasetName = sample+"_chexmix_allevents.tabular"
                        inputs[k] = inputDict[datasetName]
            ...
            for toolid , tooldict in raw_params.items():
                ...
                if tooldict['tool_id'] in confParams.keys():
                    print("\033[94m  \033[94m  Creating the param dict for : {} \033[0m \033[0m".format(tooldict['tool_id']))
                    ...
                    if len(configParams) == 2:
                        if configParams[1] == 'prot':
                            tool_inputs = {'param':configParams[0],'value':sampleList[sample][1]}
...
def createWorkflowInvokeFile(invokeInfo,sampleList):
    ...
    for run,data in invokeInfo.items():
        for sample,inputList in data.items():
            for il in inputList:
                workflowInfoFile.write("{},{},{},{},{},{},{}\n".format(run,sample,sampleList[sample][1],il['history_id'],il['workflow_id'],il['id'],il['update_time']))
...
if __name__ == '__main__':
    ...
    parser.add_argument('datasetInfo',help='datasetInfo.csv file that has dataset ids for chexmix peaks and allevents')
    args = parser.parse_args()

    # to store the samples from runInfo file, stores a list with sample numbers as keys.
    sampleList = {}
    libList = {} # toGenerate library folders
    uploadInfo = {} # to hold all the upload data, once you upload the datasets to individual sample histories.
    invokeInfo = {} # to store the info returned after invoking the workflow one each sample
    chexmixData = {} # to store the dataset ids for peaks and allevents
    ...
    # parsing the datasetInfo.csv
    with open(args.datasetInfo,'r') as dinfo:
        contents = dinfo.readlines()
        for line in contents:
            if not line.startswith('#'):
                temp = line.strip().split(',')
                if temp[0] not in chexmixData.keys():
                    chexmixData[temp[0]] = [temp[2]]
                else:
                    chexmixData[temp[0]].append(temp[2])

    # pprint.pprint(chexmixData)
    ...
    uploadLibDatasets = getLibraryDatasetsToUpload(gi,config['BASIC']['DATA_LIBRARIES'])

    # upload the necessary datasets to each history
    print("\n\033[94m  \033[95m  Uploading datasets into each sample history \033[0m \033[0m")
    uploadInfo = uploadDataToHistories(gi,libs,sampleList,chexmixData)

    # invoke the workflow.
    myWorkflow = config['BASIC']['RUN_WORKFLOW']
    print("\n\033[94m  \033[95m  Retrieving the workflow : {} \033[0m \033[0m".format(myWorkflow))
    ...
    print("\n\033[94m  \033[95m  Retrieving the inputs and parameters for the Workflow : {} \033[0m \033[0m".format(workflowID['name']))
    workflowInputs = getInputsAndParams(gi,uploadInfo,hlibs,workflowID,config['INPUTS'],config['PARAMS'],sampleList)

    print("\n\033[95m  \033[95m  Starting Workflow invokations !  \033[0m \033[0m")
    for run,data in workflowInputs.items():
        print("\033[95m  \033[94m  Starting Workflow for run : {} \033[0m \033[0m".format(run))
        ...
        for sample,inputList in data.items():
            ...
            invoked_workflow = gi.workflows.invoke_workflow(workflow_id=inputList[2], inputs=inputList[0], params=inputList[1], history_id=inputList[3])
            invokeInfo[run][sample].append(invoked_workflow)
            # pprint.pprint(invoked_workflow)
            print("\033[95m  \033[94m  Invoked Workflow for sample : {} \033[0m \033[0m".format(sample))
            print("\033[95m  \033[94m  worflow_id : {} \n    history_id : {} \n    invoke_id : {} \n    update_time : {} \033[0m \033[0m\n".format(invoked_workflow['workflow_id'],invoked_workflow['history_id'],invoked_workflow['id'],invoked_workflow['update_time']))

    print("\033[95m  \033[95m  Creating the WorkflowInvokeFile (CSV) \033[0m \033[0m")
