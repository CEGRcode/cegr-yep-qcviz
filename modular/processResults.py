...
Renames the datasets and copies them into Individual category folders.

All Samples in Results folder will be processed even if that sampleID
doesn't exist in the WorkflowInvokeFile.

WorkflowInvokeFile is only used to print info to the terminal absolute
...
if __name__ == '__main__':
    ...
    parser.add_argument('workflowInvokeFile',help='WorkflowInvokeFile generated by the yepQCVIZ pipeline for this specific processing')
    ...
    promoterPath = os.path.abspath(args.destPath)+"/Promoter"
    ...
    ltrPath = os.path.abspath(args.destPath)+"/LTR"
    RNAPol2Path = os.path.abspath(args.destPath)+"/RNAPol2"
    RNAPol3Path = os.path.abspath(args.destPath)+"/RNAPol3"
    memePath = os.path.abspath(args.destPath)+"/Meme"
    fimoPath = os.path.abspath(args.destPath)+"/FIMO"
    allgenesPath = os.path.abspath(args.destPath)+"/AllGenes"
    boundgenesPath = os.path.abspath(args.destPath)+"/BoundGenes"
    enrichedfeaturesPath = os.path.abspath(args.destPath)+"/EnrichedFeatures"
    bamPath = os.path.abspath(args.destPath)+"/Bam"
    peaksPath = os.path.abspath(args.destPath)+"/ChexmixPeaks"
    filteredPeaksPath = os.path.abspath(args.destPath)+"/FilteredPeaks"
    allEventsPath = os.path.abspath(args.destPath)+"/AllEvents"
    ...
    destinationList = [promoterPath,tssPath,tesPath,xelementPath,centromerePath,arsPath,memePath,fimoPath,allgenesPath,boundgenesPath,enrichedfeaturesPath,bamPath,peaksPath,filteredPeaksPath,subsectorPath,bigWigPath,bigBedPath,allEventsPath,ltrPath,RNAPol2Path,RNAPol3Path]
    ...
    for root, dirs, files in os.walk(srcPath, topdown=True):
        prefix = (root.split("/")[-1:][0].split('_')[0]).upper()
        # print(root.split("/"))
        ...
        for filename in files:
            if filename.startswith(prefix + "_AllGenes"):
                source = os.path.join(root, filename)
                destination = os.path.join(allgenesPath,filename)
                print("\033[94m Copying {}\033[0m".format(filename))
                shutil.copy(source,destination)
            if filename.startswith(prefix + "_BoundGenes") or filename.startswith(prefix + "_GENEMID"):
                source = os.path.join(root, filename)
                destination = os.path.join(boundgenesPath,filename)
                print("\033[94m Copying {}\033[0m".format(filename))
                shutil.copy(source,destination)
            if filename.startswith(prefix + "_EnrichedFeatures"):
                ...
                print("\033[94m Copying {}\033[0m".format(filename))
                shutil.copy(source,destination)
            if filename.startswith(prefix + "_EnrichedPromoter"):
                source = os.path.join(root, filename)
                destination = os.path.join(promoterPath,filename)
                print("\033[94m Copying {}\033[0m".format(filename))
                shutil.copy(source,destination)
            if filename.startswith(prefix + "_Promoter"):
                source = os.path.join(root, filename)
                destination = os.path.join(promoterPath,filename)
                print("\033[94m Copying {}\033[0m".format(filename))
                shutil.copy(source,destination)
            ...
            if filename.startswith(prefix + "_TES"):
                ...
                print("\033[94m Copying {}\033[0m".format(filename))
                shutil.copy(source,destination)
            if filename.startswith(prefix + "_RNAPol3"):
                source = os.path.join(root, filename)
                destination = os.path.join(RNAPol3Path,filename)
                print("\033[94m Copying {}\033[0m".format(filename))
                shutil.copy(source,destination)
            ...
            if filename.startswith(prefix + "_ChExMix_(all.tabular"):
                ...
                print("\033[94m Copying {}\033[0m".format(filename))
                shutil.copy(source,destination)
            if filename.startswith(prefix + "_RNAPol2"):
                source = os.path.join(root, filename)
                destination = os.path.join(RNAPol2Path,filename)
                print("\033[94m Copying {}\033[0m".format(filename))
                shutil.copy(source,destination)
            if filename.startswith(prefix + "_LTR"):
                source = os.path.join(root, filename)
                destination = os.path.join(ltrPath,filename)
                print("\033[94m Copying {}\033[0m".format(filename))
                shutil.copy(source,destination)
