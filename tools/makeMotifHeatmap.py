...
def sortData(sense,anti):
    ...
    combined['sum'] = combined[combined.columns[2:]].sum(axis=1) # summing each row

    # sort in the descending order of the sum for each row.
    combinedSort = combined.sort_values(by='sum',ascending=False,na_position='first')
...
if __name__ == '__main__':
    ...
    parser.add_argument('firstCatergory',help='Catergory of input (sense/anti), used to do head on data')
    parser.add_argument('sense',help='sense tabular')
    parser.add_argument('secondCatergory',help='Catergory of input (sense/anti), used to do head on data')
