...
_Installing Anaconda usually satisfies all the dependencies for most of the python scripts. pip is used to install bioblend. Make sure you have them installed before you run any of the scripts._
...
- `downloadResults.py` is the script that uses the `workflowInfo.csv` file to create the required folder structure and download the datasets into their respective sample folders. You should see `Results` folder appear in the directory where this script was run. All sample sub-folders are contained within it. Currently this script does not move the files to `/pass`, which need to be done manually. Dataset download for failed histories is skipped by default.

```
Usage:

    python downloadResults.py yepQcViz.cfg workflowInfo.csv
```
